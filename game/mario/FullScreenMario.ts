    ...
    export class FullScreenMario extends GameStartr.GameStartr implements IFullScreenMario {
        ...
        */
        ...
        gameStart(): void {
            ...
            FSM.ItemsHolder.setItem("score", (<any>FSM.settings.statistics.values).score.valueDefault);

            clearInterval((<any>window).cheepDeathInterval);
        ...
        gameOver(): void {
            ...
            if (!(<any>window).isVictory) {
                FSM.AudioPlayer.play("Game Over");
            } else {
                FSM.AudioPlayer.play("Win");
            }
            ...
            FSM.TimeHandler.addEventInterval(
                function (): void {
                    FSM.spawnRandomCheep(FSM);
                },
                1,
                15);

            (<any>window).showEndImage();
            (<any>window).$('#game').hide();
        ...
        playerShroom(thing: IPlayer, other: IItem): void {
            ...
            if (thing.power < 3) {
                ...
                if (thing.power < 3) {
                    ...
                    if (thing.power === 3) {
                        thing.FSM.ItemsHolder.increase("lives");
        ...
        */
        ...
        activateCheepsStart(thing: IDetector): void {
            ...
            thing.FSM.TimeHandler.addEventInterval(thing.FSM.spawnRandomCheep, 38, Infinity, thing.FSM);
        }
        ...
        */
        ...
        findScore(level: number): number {
            ...
            FSM.ItemsHolder.increase("score", 200);
        ...
        */
        ...
        setMap(name?: string | IFullScreenMario, location?: string | number): void {
            ...
            if (map.comics) { // requied на самом деле
                FSM.PixelDrawer.setBackground("black");
                FSM.AudioPlayer.playTheme("Underworld", true);

                (<any>window).$('#fotorama').show();
                (<any>window).$('#game').hide();
                (<any>window).$('#loadScreen').hide();
       
                (<any>window).fotorama.splice(0, (<any>window).fotorama.size);

                var photos = [];

                for (var i = 0; i < map.comics.length; i++) {
                    photos.push({ img: 'Images/Cartoons/' + map.comics[i] });
                }
                    
                photos.push({ img: 'Images/clear.gif' });

                (<any>window).fotorama.load(photos);

                (<any>window).fotoramaDiv.on('fotorama:showend', function (e, f) {
                    if (f.size == (f.activeIndex + 1)) {
                        (<any>window).$('#fotorama').hide();
                        (<any>window).$('#game').show();
                        ...
                        if (name === "2-3") {
                            FSM.AudioPlayer.playTheme("Hurry Overworld", true)
                        }
        ...
        cutsceneFlagpoleCountdown(FSM: FullScreenMario, settings: any): void {
            FSM.TimeHandler.addEventInterval(
                function (): boolean {
                    ...
                    FSM.ItemsHolder.increase("score", 25);
